...
function SQLSuccess(props: SQLSuccessProps) {
  ...
  useEffect(() => {
    ...
    fetch(
      `${NEXT_PUBLIC_API_URL}/v1/workspaces/${props.workspaceId}/documents/${props.documentId}/queries/${props.blockId}?page=${currentPageIndex}&pageSize=${rowsPerPage}&dataframeName=${props.dataframeName}`,
      {
        credentials: 'include',
      }
    )
