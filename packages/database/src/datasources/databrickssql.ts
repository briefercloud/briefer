...
export async function listDatabricksSQLDataSources(
  ...
  return fromDB.map(
    (pd): DataSource => ({ type: 'databrickssql', data: toDatabricksSQLDataSource(pd) })
  )
}
  ...
  return prisma()
    .databricksSQLDataSource.update({
      ...
      data: {
        ...data,
        token: data.token
          ? encrypt(data.token, encryptionKey)
          : undefined,
      },
    })
...
export async function deleteDatabricksSQLDataSource(
  ...
  return toDatabricksSQLDataSource(
    await prisma().databricksSQLDataSource.delete({ where: { workspaceId, id } })
  )
}